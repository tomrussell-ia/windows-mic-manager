...
public partial class MicrophoneEntryViewModel : ObservableObject
{
    ...
    public void UpdateMeter(double inputPercent)
    {
        ...
        var inputDbFs = MicrophoneManager.Core.Services.ObsMeterMath.PercentToDb(clamped);
        ...
        var smoothedPercent = MicrophoneManager.Core.Services.ObsMeterMath.DbToPercent(_smoothedDbFs);
    ...
    public void TickPeak(DateTime nowUtc)
    {
        ...
        var inputDbFs = MicrophoneManager.Core.Services.ObsMeterMath.PercentToDb(InputLevelPercent);
        _peakDbFs = Math.Max(inputDbFs, _peakDbFs);
        _peakDbFs = MicrophoneManager.Core.Services.ObsMeterMath.ClampMeterDb(_peakDbFs);

        var newPeakPercent = MicrophoneManager.Core.Services.ObsMeterMath.DbToPercent(_peakDbFs);
